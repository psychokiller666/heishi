...
$(document).on('pageInit','.store-show', function (e, id, page) {
 ...
 //领取九折优惠券
  ...
  function setDescImgWH1(){
    var $imgWrap = $('.js_desc_wrap2').find('img');

    if($imgWrap.length>0){
      var $descWrap = $imgWrap.parents('.js_desc_wrap2');
      var descW = $descWrap.width()*100/92
      if(descW>0){
...
/*  var attention = $('.attention');
  ...
  function initAlikeContent(data){
      ...
      data.forEach(t=>{
        ...
        if(t.type==2&&!(t.url_type==19||t.url_type==16||t.url_type==17||t.url_type==18||t.url_type==2||t.url_type==3||t.url_type==74)){
          list+=`
